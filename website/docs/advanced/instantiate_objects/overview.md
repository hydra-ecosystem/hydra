...
Hydra does not authorize `builtins.getattr` through a real target whitelist.
Attribute access can execute property or custom descriptor code before Hydra
can inspect the returned value. Access the intended attribute from trusted
Python code instead. On the legacy no-whitelist path, non-callable values are
returned normally and callable results are checked against the blocklist. The
explicit `UNSAFE_ALLOW_ALL_TARGETS` escape hatch preserves unrestricted legacy
behavior.

Hydra also refuses to authorize `operator.call`, `operator.attrgetter`, and
`operator.methodcaller`, which delegate callable invocation, attribute
selection, or method selection to config data. Set `_target_` to the intended
callable instead. These generic operator targets are also blocklisted on the
legacy no-whitelist path.
