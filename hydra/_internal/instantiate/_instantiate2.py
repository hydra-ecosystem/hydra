...
def _resolve_target(
    ...
    if isinstance(target, str) or callable(target):
        ...
        # This exact deprecated no-op emits its own migration warning.
        if (
            execution_whitelist is None
            and target_name != "hydra.experimental.callbacks.LogJobReturnCallback"
        ):
