...
class HydraDictConfigurator(logging.config.DictConfigurator):
    ...
    def configure_handler(self, config: Any) -> Any:
        ...
        resolved_from = handler_class if isinstance(handler_class, str) else ""
        for key in ("class", "queue", "listener"):
            value = config.get(key)
            if callable(value):
                target_name = self._authorize_callable(value, "")
                if key == "class":
                    resolved_from = target_name
